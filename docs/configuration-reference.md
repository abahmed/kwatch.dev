...
description: Complete kwatch configuration key, type, default, and status reference generated from the current config catalog
keywords: [kwatch configuration, Kubernetes monitoring settings, alert configuration, config catalog]
---
...
For practical explanations and examples, start with [General configuration](/docs/general-configuration). Apply configuration through [`kwatch.sh`](/docs/kwatch-manager) and validate it with [`kwatch lint`](/docs/cli-commands).
